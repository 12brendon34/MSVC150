...
void CFrameImpl::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI)
{
	...
	if ((m_pFrame->GetStyle() & WS_CAPTION) == 0 || (m_pFrame->GetStyle() & WS_BORDER) == 0)
	{
		...
		if (afxGlobalData.m_nShellAutohideBars & AFX_AUTOHIDE_BOTTOM)
		{
			rect.bottom -= 2;
		}

		if (afxGlobalData.m_nShellAutohideBars & AFX_AUTOHIDE_TOP)
		{
			rect.top += 2;
		}

		if (afxGlobalData.m_nShellAutohideBars & AFX_AUTOHIDE_RIGHT)
		{
			rect.right -= 2;
		}

		if (afxGlobalData.m_nShellAutohideBars & AFX_AUTOHIDE_LEFT)
		{
			rect.left += 2;
		}
